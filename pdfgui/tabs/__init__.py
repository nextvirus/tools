...
照片换底、会议纪要、PDF 水印与导出图片均依赖可选安装组件；主窗口在**某路探测成功时**即调用对应
`mount_*` 挂上标签（先出先有）。探测过程中**只更新顶部「模块检测」文案**；三路都结束后收起状态行、将 Notebook
提到首行，并**最多两次**收尾布局（浅 + 深，避免客户区高度为 0）。
`mount_all_tabs` 仍保留，供需要「按探测结果一次性挂载」的入口复用。
"""
